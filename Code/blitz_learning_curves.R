...
defense <- defense %>% 
  select(role,
         ...
         test_weeks,
         id_all_frame) %>% 
  mutate(role=ifelse(role=='Pass Rush','PassRush',role))
...
pred <- mod$pred$PassRush
obs <- ifelse(mod$pred$obs=='PassRush',1,0)

roc_obj <- roc(obs,pred)
rp <- ggroc(roc_obj)

ggsave(paste0(savepath,'blitz_ROC.png'),plot = rp)
