...
pacman::p_load(pacman,tidyr,caret,mltools,caretEnsemble,recipes,expss,purrr,stringr,dplyr,tibble,ggplot2,doParallel,gganimate,nflfastR,ggimage,gifski,grid,flextable,gtools,zoo)
...
#A Gap left LOS players
weeks$onLOS_Agap_left_forOffense <- vlookup(lookup_value = weeks$id,
                                            dict = weeks %>% 
                                              group_by(id) %>% 
                                              mutate(Agap=ifelse(time==snap_time & 
                                                                   onLOS==1 & 
                                                                   defenderGap=='A Gap Left',
                                                                 1,0)) %>% 
                                              summarize(Agap=max(Agap)),
                                            result_column = 2,
                                            lookup_column = 1)

#A Gap right LOS players
weeks$onLOS_Agap_right_forOffense <- vlookup(lookup_value = weeks$id,
                                            dict = weeks %>% 
                                              group_by(id) %>% 
                                              mutate(Agap=ifelse(time==snap_time & 
                                                                   onLOS==1 & 
                                                                   defenderGap=='A Gap Right',
                                                                 1,0)) %>% 
                                              summarize(Agap=max(Agap)),
                                            result_column = 2,
                                            lookup_column = 1)
...
# find rate of pressure grouped by 2nd level blitz opp or same side as center turn

press_rate <- 
left_join(x = weeks %>% 
            
            mutate(left=ifelse(role=='Pass Rush' & 
                                 position_general %in% c('B','$','C') & 
                                 # str_detect(defenderGap,'Left') &
                                 (str_detect(defenderGap_1_sec,'Left') | 
                                    str_detect(defenderGap_1_sec,'Head Up on L')),1,0),
                   right=ifelse(role=='Pass Rush' & 
                                  position_general %in% c('B','$','C') &
                                  # str_detect(defenderGap,'Right') &
                                  (str_detect(defenderGap_1_sec,'Right') | 
                                    str_detect(defenderGap_1_sec,'Head Up on R')),1,0)) %>% 
            
            mutate(oppCT=ifelse((left==1 & centerTurn=='right') | 
                                  (right==1 & centerTurn=='left'),1,0),
                   sameCT=ifelse((left==1 & centerTurn=='left') | 
                                   (right==1 & centerTurn=='right'),1,0)) %>% 
            
            mutate(blitzCenterTurn=ifelse(oppCT==1,'opp',ifelse(sameCT==1,'same',''))) %>% 
            
            select(oppCT,sameCT,blitzCenterTurn,id_all,id),
            
          
          y = pffScoutingData %>% mutate(hhs=ifelse(pff_hit==1 |
                                                    pff_hurry==1 |
                                                    pff_sack==1,1,0)) %>% select(id_all,hhs),
          by='id_all') %>% 
  filter(is.na(hhs)==F & blitzCenterTurn!='') %>%
  distinct(id_all,hhs,blitzCenterTurn) %>% 
  group_by(blitzCenterTurn) %>% 
  count(hhs)
...
p_opp <- ggplot(press_rate %>% filter(blitzCenterTurn=='opp') %>% mutate(hhs=as.factor(hhs)), aes(x='', y=n, fill=hhs)) +
  scale_fill_manual(values=c('dodgerblue','firebrick')) +
  geom_bar(stat="identity", width=1) +
  coord_polar("y", start=0) +
  geom_label_repel(aes(y = n/2, label = ifelse(hhs==1,paste0(percent(n/sum(n),accuracy = 0.1), '\nPressure'),'')),
                   size = 10, nudge_x = 1, show.legend = FALSE, col='black',fill='white')+
  ...
  ggtitle('Blitz OPPOSITE Center Turn')




p_same <- ggplot(press_rate %>% filter(blitzCenterTurn=='same') %>% mutate(hhs=as.factor(hhs)), aes(x='', y=n, fill=hhs)) +
  scale_fill_manual(values=c('dodgerblue','firebrick')) +
  geom_bar(stat="identity", width=1) +
  coord_polar("y", start=0) +
  geom_label_repel(aes(y = n/2, label = ifelse(hhs==1,paste0(percent(n/sum(n),accuracy = 0.1), '\nPressure'),'')),
  ...
  theme(legend.position='none',
        ...
        axis.ticks = element_blank(),
        plot.title = element_text(hjust = 0.5,face='bold',size=30)) +
  ggtitle('Blitz SAME SIDE as Center Turn')
...
ggsave(paste0(savepath,'blitz_opp_same_press_rate.png'),
       plot = p_opp + p_same +
...
# density plot for pressure frequency and blitz probability for pass rushers
blitzprob_pressfreq_density_yespressure <- 
  left_join(x = defense %>% filter(test_weeks==1 & role=='Pass Rush' & position_general %in% c('B','$','C')) %>% 
            mutate(prob=round(blitzprob_pred_PassRush,2)),
          y = pffScoutingData %>% mutate(hhs=ifelse(pff_hit==1 |
                                                      pff_hurry==1 |
                                                      pff_sack==1,1,0)) %>% select(id_all,hhs),
          by = 'id_all') %>% 
  filter(hhs==1) %>% 
  select(BlitzProb=blitzprob_pred_PassRush)


ggplot(blitzprob_pressfreq_density_yespressure,aes(BlitzProb)) + 
  # geom_point(aes(x=prob,y=Pct)) +
  geom_density(fill='dodgerblue',adjust=0.75)


blitzprob_pressfreq_density_nopressure <- 
  left_join(x = defense %>% filter(test_weeks==1 & role=='Pass Rush' & position_general %in% c('B','$','C')) %>% 
              mutate(prob=round(blitzprob_pred_PassRush,2)),
            y = pffScoutingData %>% mutate(hhs=ifelse(pff_hit==1 |
                                                        pff_hurry==1 |
...
ggplot(blitzprob_pressfreq_density_nopressure,aes(BlitzProb)) + 
  # geom_point(aes(x=prob,y=Pct)) +
  geom_density(fill='dodgerblue',adjust=0.75)
  


# calculate pressure frequency based on blitz probability for all pass rushers



# only include pre-snap data & defensive players
defense <- weeks %>% filter(frameId<=snap_frame &
                              off_def=='D')

defense$test_weeks <- ifelse(defense$gameId %in% (plays %>% filter(week >= 6) %>% select(gameId) %>% unname %>% unlist)[1],1,0)



blitzprob_pressfreq_alldata <- 
  left_join(x = defense %>% filter(role=='Pass Rush') %>% 
              mutate(prob=round(blitzprob_pred_PassRush,2)),
            y = pffScoutingData %>% mutate(hhs=ifelse(pff_hit==1 |
                                                        pff_hurry==1 |
                                                        pff_sack==1,1,0)) %>% select(id_all,hhs),
  ...
  summarize(hhs=sum(hhs),
            plays=length(prob),
            dist=mean(abs(LOS-x))) %>% 
  mutate(Pct=hhs/plays) %>% 
  filter(plays>=1) %>% 
  # mutate(prob_bins=as.factor(case_when(prob>=0.9 ~ '90-100%',
  #                  prob>=0.8 ~ '80-90%',
  #                  prob>=0.7 ~ '70-80%',
  #                  prob>=0.6 ~ '60-70%',
  #                  prob>=0.5 ~ '50-60%',
  ...
  #                  prob>=0.1 ~ '10-20%',
  #                  TRUE ~ '0-10%'))) %>% 
  mutate(Probability=as.factor(case_when(prob>=0.9 ~ '90-100%',
                                       prob>=0.8 ~ '80-89%',
                                       prob>=0.5 ~ '50-79%',
                                       prob>=0 ~ '0-49%',
                                       TRUE ~ ''))) %>% 
  select(`Blitz Probability`=prob,
         `Pressure Frequency`=Pct,
         Probability,
         hhs,
         plays,
         dist) %>% 
  mutate(bins=bin_data(`Blitz Probability`,bins = 20)) %>% 
group_by(bins) %>%
summarize(hhs=sum(hhs),
          plays=sum(plays),
          `Blitz Probability`=mean(`Blitz Probability`),
          dist=mean(dist)) %>%
mutate(`Pressure Frequency`=hhs/plays) %>%
as.data.frame()



blitzProb_pressFreq <- 
ggplot(blitzprob_pressfreq_alldata %>% 
         filter(`Blitz Probability`>=0.45),aes(x=`Blitz Probability`,y=`Pressure Frequency`))+
  geom_col(aes(x=`Blitz Probability`,y=`Pressure Frequency`),fill='dodgerblue')+
  # geom_text(aes(x=`Blitz Probability`,y=`Pressure Frequency`,label=paste0(hhs,' / ',plays)),col='black',vjust=-0.8)+
  geom_smooth(method='glm',linewidth=3,col='firebrick')+
  scale_y_continuous(labels = scales::percent_format(),breaks = seq(0,0.5,0.1))+
  scale_x_continuous(labels = scales::percent_format(),breaks = seq(0.5,1,0.1))+
  ylab('Pressure Frequency\n(hits + hurries + sacks) / (plays)')+
  # ggtitle('Why Can We Use Blitz Probability To Maintain a Stealthy Attack?')+
...
ggsave(paste0(savepath,'blitzProb_pressFreq.png'),plot = blitzProb_pressFreq)
