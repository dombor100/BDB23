...
data_density <- data_density %>% filter(totalDL==5)
  ...
  p_right <- 
    ...
    scale_fill_gradientn(colors=c('black','dodgerblue4','darkred','firebrick','red','purple','white')) +
      ...
      geom_text(data=data_density %>% filter(centerTurn=='right' & off_def=='O') %>% group_by(position) %>% summarize(x=mean(x),y=mean(y)),
                 aes(x=y,y=x,label=position),size=8) +
  ...
  ggsave(paste0(savepath,'defensive_front_density.png'),
         plot = p_left + p_right + plot_layout(nrow=2,byrow = F),width=10,height=18)
