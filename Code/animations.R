...
id3 <- '2021101710_1202'
  # plays %>% filter(possessionTeam=='NE' & defensiveTeam=='DAL' & id %in% weeks$id) %>% select(id) %>% unlist %>% unname
...
for(j in 1:length(ids)){
  ...
  week <- plays %>% filter(id==play) %>% mutate(w=paste0('Week ',week,': ',offense,' (offense) vs ',defense,' (defense)')) %>% select(w) %>% unlist %>% unname
  ...
  p <- 
    ...
    geom_point(data=data_snap %>% filter(off_def!='D'),
               ...
               fill=ifelse((data_snap %>% filter(off_def!='D') %>% select(off_def) %>% unname %>% unlist)=='ball','darkorange',
                           ifelse((data_snap %>% filter(off_def!='D') %>% select(off_def) %>% unname %>% unlist)=='O','gray','white')),
               alpha=ifelse((data_snap %>% filter(off_def!='D') %>% select(off_def) %>% unname %>% unlist)=='OL',1,0.4), size=12) +
    ...
    geom_point(data=data_snap %>% filter(off_def=='D'),
               aes(x=y,y=x,shape='21',group=nflId,color=blitzprob_pred_PassRush,fill=blitzprob_pred_PassRush),size=12) + 
    ...
    geom_segment(data=data_snap %>% filter(position=='C' & freeze==0),aes(x=y,xend=y,y=x-0.5,yend=x-2),size=1.5,color='white') +
    ...
    geom_text(data=data_snap %>% filter(position=='C' & freeze==0),aes(x=y,y=x-2.5,label=centerturn_pred_pct),
              size=4,hjust='center',color='white')+
    ...
    geom_label(aes(x=51.333333,y=smallest_yardLine+5,label=paste0('Time to Snap = ',round(timeToSnap,1),'\n',
    ...
    # defender gaps
    geom_text(aes(x = y, y = x+1, label = ifelse(inWideBox==1,ifelse(str_detect(defenderGap,'Open')==T,'Open',str_extract(defenderGap,'.')),NA)),
              colour = "white", 
              vjust = 0.36, size = 3.5) + 
    ...
    geom_label(aes(x=51.333333,y=ymax-5,label=paste0(week,'\n',score,'\n',clock,'\n',dd,'\n',LOS_rel)),hjust='left',size=5)+
  ...
  #save animation
  anim_save(paste0(savepath,'anim',j,".gif"),
